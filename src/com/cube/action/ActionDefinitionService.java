...
public class ActionDefinitionService {
	...
	private ActionDefinition getActionDefinition(int id) {
		...
		String query = null;
		if (id == 2) {
			query = "SELECT event FROM com.cube.entity.Event event WHERE event.noun!='fdbk' and event.eventId=:TriggeringEventId and event.userid = :TriggeringUserId";
			actionDefinition.setExecuteAfter(15);
			...
			query = "SELECT SUM(properties.rhs) FROM com.cube.entity.Properties properties, com.cube.entity.Event event WHERE event.noun='bill' and event.verb='pay' and event.userid = :TriggeringUserId AND properties.lhs='value' and properties.eventId=event.eventId and event.creationTime>=:CreationTime GROUP BY event.userid HAVING SUM(properties.rhs) >= 20000";
			actionDefinition.setExecuteAfter(5);
			actionDefinition.setQuery(query);
